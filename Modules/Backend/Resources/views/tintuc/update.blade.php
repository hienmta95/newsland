    ...
    <div class="sp-push-form">
        <form action="{{ route('backend.tintuc.update', $tintuc->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group @if (count($errors->all())) {{$errors->has(['image']) ? 'has-error' : 'has-success'}} @endif">
                <div>
                    <img class="show-images"  class="img-thumbnail" src="{!! $tintuc->image ? asset('/').$tintuc->image->url : ""!!}" alt="web_image" title="image">
                </div>
            ...
            <div class="form-group @if (count($errors->all())) {{$errors->has(['noidung']) ? 'has-error' : 'has-success'}} @endif">
                ...
                <textarea id="noidung_tintuc" class="form-control{{ $errors->has('noidung') ? ' has-error' : '' }}" name="noidung" maxlength="255" rows="3">{{ $tintuc->noidung }}</textarea>
                <script type="text/javascript">
                    var editor = CKEDITOR.replace('noidung_tintuc',{
                        language:'vi',
            ...
            <div class="form-group @if (count($errors->all())) {{$errors->has(['tomtat']) ? 'has-error' : 'has-success'}} @endif">
                ...
                <textarea id="tomtat" class="form-control{{ $errors->has('tomtat') ? ' has-error' : '' }}" name="tomtat" maxlength="255" rows="3">{{ $tintuc->tomtat }}</textarea>
                <script type="text/javascript">
